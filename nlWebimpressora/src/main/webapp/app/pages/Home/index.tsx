...
const generateColumns = (
  ...
  },
  {
    ...
    flex: 1,
    align: 'right',
    renderCell: (p: GridRenderCellParams<Row>) => currencyFormatter.format(p.value),
  },
  {
    field: 'quantidade',
    ...
    renderCell: params => {
      ...
      return (
        <TextField
          ...
          type="number"
          fullWidth
          onKeyDownCapture={e => (e.key === 'ArrowUp' || e.key === 'ArrowDown') && e.stopPropagation()}
          inputProps={{
            style: {
          ...
          onChange={e => onChangeField('quantidade', Number(e.target.value), params.row.id)}
        />
      );
    },
  },
  {
    ...
    headerAlign: 'right',
    align: 'right',
    renderCell: (p: GridRenderCellParams<Row>) => currencyFormatter.format(p.row.preco * p.row.quantidade),
  },
  {
    field: 'imprimir',
    ...
    renderCell: p => {
      ...
      return (
        <Tooltip title={hasChanged ? tooltipText : ''} placement="top" arrow>
          <div className={`transition-all ${hasChanged ? 'opacity-1' : 'opacity-0'}`}>
            <IconButton onClick={() => hasChanged && handlePrint(p.row)}>
              {isMoreThanBefore && <FiPrinter className="text-blue-500" size={20} />}
              {isFewerThanBefore && <FiSave className="text-red-500" size={20} />}
            </IconButton>
          </div>
        </Tooltip>
      );
...
function Home() {
  ...
  const total = products.reduce((acc, curr) => acc + curr.preco * curr.quantidade, 0);
  ...
  const editFieldRow = (field: keyof Row, value: any, id: Row['id']) => {
    ...
    if (!(edited.id in originalRows)) {
      ...
      setEditedRows(state =>
        state.map(alreadyEditedRow => (alreadyEditedRow.id === edited.id ? edited : alreadyEditedRow)),
      );
    }
  ...
  const handleConfirmPrintAll = async (products: Row[]) => {
    const confirmation = await confirmationToast('Você tem certeza que deseja imprimir TODaS as impressões pendentes?');
    ...
    const message =
      qtdToPrint < 0 ? (
        <>
          ...
          <span>
            Você está diminuindo {Math.abs(qtdToPrint)} unidade(s) do produto "{product.nome}". Você tem certeza de que
            deseja fazer isso?
          </span>
        </>
      ) : (
        `Tens certeza que deseja imprimir ${qtdToPrint} unidade${qtdToPrint > 1 ? 's' : ''} de ${product.nome}?`
      );
  ...
  const handlePrint = async (productsToPrint: Row[]) => {
    ...
    setOriginalRows(state => {
      return Object.fromEntries(
        Object.entries(state).filter(([key]) => !productsToPrint.some(prod => prod.id === Number(key))),
      );
    });
    setEditedRows(state => state.filter(editedProd => !productsToPrint.some(prod => prod.id === editedProd.id)));
  };
    ...
    <>
      ...
      <main className="p-4">
        ...
        <div className="flex items-end justify-end">
          <Button style={{ marginRight: '50px' }} variant="contained" onClick={handleResume}>
            Resumo
          </Button>
        </div>
        ...
        <footer className="fixed bottom-1 right-4">
          <span className="text-gray-500 text-xs">
            Feito por Gustavo André Cauzzi, Bruno Ascari, João Vitor Bufon de Almeida e Matheus Pereira Gomes da Silva
            como cortesia da família NL ❤
          </span>
        </footer>
      </main>
    </>
...
const PrintAllButton: React.FC<PrintAllButtonProps> = ({ editedProducts, originalProducts, onClick, ...props }) => {
  const productsToPrint = editedProducts.filter(
    editedProduct => editedProduct.quantidade > originalProducts[editedProduct.id].quantidade,
  );
  ...
  return (
    <Tooltip title={shouldRender ? 'Imprimir todos' : ''} arrow placement="top">
      <div className={`transition-all ${shouldRender ? 'opacity-1' : 'opacity-0'}`}>
        <IconButton {...props} disabled={!shouldRender} onClick={() => onClick(productsToPrint)}>
